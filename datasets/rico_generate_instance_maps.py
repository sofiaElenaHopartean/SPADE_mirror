...
parser.add_argument('--in_semantic_json_dir', type=str, default="./rico/semantic_ann_json/",
...
parser.add_argument('--out_ann_cats_csv', type=str, default="./rico/ann_cat.csv",
                    help="Path to the annotation categories csv file")
parser.add_argument('--out_dir', type=str, default="./rico/",
...
'''
save_imgs = True
out_folder = opt.out_dir+"semantic_ann_img"
...
for i in range(nb_images):
    ...
    with open(file_name,"r") as read_json:
        ...
        if not save_imgs: 
            continue
        cv2.imwrite(out_folder+"/"+str(imgs_list[i])+".jpg",cv2.resize(img,(desired_width,desired_height)))
        
# print(imgs_list[:10])
print(label_color_dict.items())
print("removed imgs:",nb_removed_imgs)
...
'''
save_imgs = True
out_folder = opt.out_dir+"instance_map_img"
if not Path(out_folder).is_dir(): 
    try:  
        os.mkdir(out_folder)
    except OSError:  
...
'''
save_imgs = True
out_folder = opt.out_dir+"screenshots_resized"
if not Path(out_folder).is_dir(): 
    try:  
        os.mkdir(out_folder)
    except OSError:  
...
for i in range(nb_images): 
    ...
    sem_img = Path(opt.out_dir+"semantic_ann_img/"+str(imgs_list[i])+".jpg")
    res_img = Path(opt.out_dir+"screenshots_resized/"+str(imgs_list[i])+".jpg")
